...
class EventsChangePast extends Component {
    constructor() {
        super();
...
q
    ...
    handleDeleteEvent = (e) => {
        ...
        if(this.state.eventOne === this.state.eventTwo) {
            if(this.state.eventOne && this.state.eventTwo) {
                firestoreDb.collection('events').doc(this.state.eventOne).delete().then( () => 
    ...
    handleAddEvent = (e) => {
        ...
        e.preventDefault();
        if(this.state.name) {
            firestoreDb.collection('events').doc(this.state.name).set({
                name: this.state.name,
                participants: this.state.participants,
                hosts: this.state.hosts,
                location: this.state.location, 
                beerpongWinner: this.state.beerpongWinner,
                smashWinner: this.state.smashWinner,
                date: new Date()
        ...
        if(this.state.name) {
            firestoreDb.collection('events').doc(this.state.name).get().then(doc => {
                if(doc.exists) {
                    this.setState({ 
    ...
    updateField = (e) => {
        ...
        e.preventDefault();
        firestoreDb.collection('events').where('name', '==', this.state.name).get().then(snap => {
            if(snap && this.state.name) {
                snap.forEach(currentDoc => {
                    if(currentDoc) {
                            firestoreDb.collection('events').doc(currentDoc.id).update({
                    ...
                    <div className="updateFormDivInner">
                        <form id="updateForm" onSubmit={this.handleAddEvent}> 
                            ...
                            <div className="updateStatsInput">
                                <label>Name (past event)</label>
                                <input type="text" name="name" onChange={this.handleChange} autoComplete="off" />
                            </div>
                            <div className="updateStatsInput">
                                <label>Participants (#)</label>
                                <input className="numberInputInStatsChange" type="number" name="participants" onChange={this.handleChange} autoComplete="off" min="0" max="3000"/>
                            </div>
                            <div className="updateStatsInput">
                            ...
                            <div className="updateStatsInput">
                                <label>Beerpong winners</label>
                                <input type="text" name="beerpongWinner" onChange={this.handleChangeString} autoComplete="off" />
                            </div>
                            <div className="updateStatsInput">
                                <label>Smash winners</label>
                                <input type="text" name="smashWinner" onChange={this.handleChangeString} autoComplete="off" />
                            </div>
...
export default EventsChangePast;